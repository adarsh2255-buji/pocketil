...
const AdminDashboard = () => {
  ...
  const [activeView, setActiveView] = useState('dashboard'); // 'dashboard', 'approvals', 'teachers', 'batches', 'students'
  ...
  const [loadingAllStudents, setLoadingAllStudents] = useState(false); // Loading state for all students
  ...
  useEffect(() => {
    ...
    else if (activeView === 'students') fetchAllStudents(); // Fetch students when tab active
  }, [activeView]);

  // Auto-fetch students when class selected in Batch Create/Edit
  useEffect(() => {
  ...
  const handleBatchSubmit = async (e) => {
    ...
    try {
      if (batchViewMode === 'create') {
        ...
        const response = await api.put(`/batches/${selectedBatch._id}`, {
          name: batchForm.name,
          studentIds: selectedStudentIds 
        });
        if (response.status === 200) {
      ...
      return (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            >
              <PlusCircle className="h-5 w-5" /> Create Batch
            </button>
          </div>
          ...
          {loadingBatches ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {batches.map(batch => (
                <div key={batch._id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:border-indigo-200 transition-colors group relative">
                  <div className="flex justify-between items-start mb-4">
                    <div className="bg-orange-50 p-3 rounded-lg">
                      <GraduationCap className="h-6 w-6 text-orange-600" />
                    </div>
                    <span className="text-xs font-semibold bg-gray-100 text-gray-600 px-2 py-1 rounded-full">Class {batch.className}</span>
                  </div>
                  <h3 className="text-lg font-bold text-gray-900 mb-1">{batch.name}</h3>
                  <p className="text-gray-500 text-sm mb-4">{batch.students?.length || 0} Students</p>
                  
                  <button 
                    onClick={() => initEditBatch(batch)}
                    className="w-full py-2 bg-gray-50 text-gray-700 rounded-lg text-sm font-medium hover:bg-indigo-50 hover:text-indigo-700 transition-colors flex items-center justify-center gap-2"
                  >
                    View & Edit Students <ChevronRight className="h-4 w-4" />
                  </button>
                </div>
              ))}
            </div>
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => setBatchViewMode('list')} className="text-gray-500 hover:text-gray-700"><ArrowLeft className="h-6 w-6" /></button>
            <h2 className="text-lg font-bold text-gray-900">
              {batchViewMode === 'create' ? 'Create New Batch' : `Edit ${selectedBatch?.name}`}
            </h2>
          </div>
        </div>
        ...
        <div className="p-6">
          {msg.text && (
            <div className={`mb-6 p-4 rounded-lg flex items-center gap-2 ${msg.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
              {msg.type === 'success' ? <CheckCircle2 className="h-5 w-5" /> : <AlertTriangle className="h-5 w-5" />}
              {msg.text}
            </div>
          )}
          ...
          <form onSubmit={handleBatchSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Class X - Morning"
                />
              </div>
              <div>
                ...
                <select
                  required
                  disabled={batchViewMode === 'edit'} 
                  className="mt-1 block w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm disabled:bg-gray-100"
            ...
            {batchViewMode === 'edit' && selectedBatch?.students?.length > 0 && (
              <div className="border rounded-md p-4 bg-indigo-50 border-indigo-100">
                <h3 className="text-sm font-bold text-indigo-900 mb-3">Students Currently in Batch</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {selectedBatch.students.map(s => (
                    <div key={s._id} className="flex items-center gap-2 text-sm text-indigo-800">
                      <CheckCircle2 className="h-4 w-4 text-indigo-600" /> 
                      {s.firstName} {s.lastName} <span className="opacity-70">({s.registerNumber})</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {batchForm.className && (
              <div className="border rounded-md p-4 bg-gray-50">
                <h3 className="text-sm font-medium text-gray-700 mb-3 flex justify-between items-center">
                  <span>{batchViewMode === 'create' ? 'Select Students' : 'Add New Students'}</span>
                ...
                {loadingStudents ? (
                  ...
                  <div className="text-center py-4 text-gray-500 text-sm">No new students found in Class {batchForm.className}.</div>
                ) : (
                  <div className="max-h-48 overflow-y-auto space-y-2 bg-white p-2 rounded border border-gray-200">
                    {availableStudents
                      .filter(s => batchViewMode === 'create' || !selectedBatch.students.some(existing => existing._id === s._id))
                      .map(student => (
                      <div 
                        key={student._id} 
                        className={`flex items-center gap-3 p-2 rounded cursor-pointer border ${selectedStudentIds.includes(student._id) ? 'bg-indigo-50 border-indigo-200' : 'hover:bg-gray-50 border-transparent'}`} 
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">Reg: {student.registerNumber}</p>
                        </div>
                      </div>
                    ))}
                    {availableStudents.filter(s => batchViewMode === 'create' || !selectedBatch.students.some(existing => existing._id === s._id)).length === 0 && (
                       <p className="text-center text-sm text-gray-400 py-2">All students in this class are already in this batch.</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="flex items-center justify-center py-2 px-6 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 transition-colors"
              >
                {submitting ? <Loader2 className="animate-spin h-5 w-5" /> : (batchViewMode === 'create' ? 'Create Batch' : 'Update Batch')}
              </button>
            </div>
          </form>
        </div>
  ...
  const renderStudentsView = () => (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-6 border-b border-gray-100 flex justify-between items-center">
        ...
        <button onClick={fetchAllStudents} className="text-sm text-indigo-600 hover:text-indigo-800 font-medium">Refresh</button>
      </div>
      
      {loadingAllStudents ? (
        <div className="p-12 text-center text-gray-500">Loading student records...</div>
            ...
            <tbody className="divide-y divide-gray-100">
              {allStudents.map((student) => (
                <tr key={student._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-600 font-medium">{student.firstName} {student.lastName}</td>
                  <td className="px-6 py-4 text-gray-500 text-sm">{student.className || '-'}</td>
                  <td className="px-6 py-4 text-gray-500 text-sm">{student.phoneNumber || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-white border-r border-gray-200 flex flex-col transition-transform duration-300 ease-in-out transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
        ...
        <div className="flex-1 py-6 px-4 space-y-1 overflow-y-auto">
          ...
          <button onClick={() => { setActiveView('batches'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-colors ${activeView === 'batches' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-50'}`}>
             <BookOpen className="h-5 w-5" /> Batches
          </button>

          <button onClick={() => { setActiveView('students'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-colors ${activeView === 'students' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-50'}`}>
        ...
        <header className="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-4 sm:px-8">
           <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-gray-800 truncate">
                ...
                {activeView === 'batches' && 'Manage Batches'}
